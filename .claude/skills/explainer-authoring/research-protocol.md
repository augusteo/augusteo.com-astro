...
## Phase 7 link + citation audit

The freshness pass above checks *when* a source is from. This pass checks that every link in the finished post actually goes where it claims and says what the prose says it says. Run it in Phase 7, alongside or right after the freshness re-check, before Gate 2. It is mandatory; a post can pass freshness and still ship a spliced quote, a mis-attributed number, or a citation whose relevance no reader can follow.

**Fan out and fetch every link.** Collect every external URL in the post (a `grep` for `](http` plus the `## References` block). Dispatch parallel agents (per `superpowers:dispatching-parallel-agents`, same budget shape as Phase 2) to WebFetch each one. For each link, the agent reports against the citing sentence:

1. **Resolves + identity.** The URL loads and points to the claimed work: title, authors, year, and venue all match the inline mention and the References entry. (This catches the MO-GRPO case: cited "Liu et al" but authored by Ichihara et al.)
2. **Quote fidelity — verbatim or not quoted.** Every excerpt in quotation marks must appear in the source verbatim. No splicing phrases from different sentences into one quoted string, no quoting a paraphrase. If you can't quote it word-for-word, drop the quotation marks and assert it in your own voice. (This catches the DPO case: the "eliminating the need for fitting a reward model, sampling..., or ...tuning" quote spliced in "fitting a reward model," which wasn't in that sentence.)
3. **Number attribution.** A figure or statistic must be cited to the source that actually states it, not an adjacent source. If paper B reports a number about dataset A, attribute the number to B, not to A's own paper. (This catches the Pick-a-Pic case: the 851K figure is the Diffusion-DPO paper's count, not the Pick-a-Pic paper's own ~500K headline.)
4. **Claim support.** Read in context, does the source actually support the citing claim, or is it merely adjacent? Re-confirm the load-bearing matrix rows here, not just that the link resolves.

**Body-only claims you can't re-verify at ship time.** If a load-bearing number lived only in a paper *body* captured during Phase 2, and at ship time the body is unfetchable (oversized PDF, broken HTML mirror) while the abstract or first-party source gives only a weaker or combined claim, publish the verbatim-traceable version and drop the unverifiable specific. Annotate the matrix row with why, and leave a re-add note. Truthfulness-first: a number you cannot currently trace to its source is treated as untrue. (This catches the SAM 3D case: abstract says a combined "at least 5:1 on objects and scenes"; a separate "6:1 scenes" sat only in the unfetchable body, so the post ships the 5:1.)

**Relevance clarity — the reader test.** For every citation, the prose must make clear *why* the source is relevant, especially when it comes from an adjacent area. A non-RL data-annotation paper in an RL post, a 2D result in a 3D post, a theory paper behind an applied claim: each needs one clause that bridges it to the throughline. The test is Vic's: *if a knowledgeable reader can't easily tell why a link is there, that is a defect to fix in the prose, not a citation to defend.* When in doubt, name what the source does and does not do (e.g. "this is a data-collection protocol, not a training method"). (This catches the pointillism case: a segmentation-labeling paper with no RL in it, cited for the cheap-verdict data economics, which confused the reader until the prose said so plainly.)

Findings feed straight into edits: requote, re-attribute, soften, or clarify. Then Gate 2 (finding category 11, citation fidelity and relevance) runs over the result as the backstop. JSTOR / paywalled / bot-blocked sources that 403 to WebFetch are verified by DOI / stable-ID metadata cross-check instead; note the link itself is still correct.
