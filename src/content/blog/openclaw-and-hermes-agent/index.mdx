...
pubDate: 2026-05-09
heroImage: "@assets/blog/openclaw-and-hermes-agent/hero.jpeg"
heroAlt: "Watercolor sketch of three brass dials labeled lifespan, surface, and adaptation, flanked by a lobster, a caduceus with a four-arc closed loop, and a crumpled paper ladder."
tags: ["Tech", "AI"]
featured: false
draft: true
essay: true
...
*A survey of open-source AI agent frameworks along three architectural dials (lifespan, surface, and adaptation), placing OpenClaw, Hermes Agent, and the Claw family variants on the same map. About a 35-minute read.*
...
{/* Reader can now: see how third-party taxonomies fill a vacuum when projects share a name pattern but no maintainership; recognize that the three-dial framework itself doesn't explain every variant's distinctive bet (and that's part of the taxonomy-failure story). */}
...
<text x="300" y="162" font-family="JetBrains Mono" font-size="10" fill="#1A1A1A" text-anchor="middle" letter-spacing="1.2">FTS5-INDEXED SESSION STORE</text>
<text x="300" y="176" font-family="serif" font-size="9" font-style="italic" fill="#6B6258" text-anchor="middle">past sessions, indexed for full-text query</text>

<rect x="60" y="190" width="150" height="40" fill="#EDE5D4" stroke="#8F8578" stroke-width="0.8" rx="2"/>
<text x="135" y="206" font-family="JetBrains Mono" font-size="9" fill="#1A1A1A" text-anchor="middle">session-aaa</text>
...
<text x="300" y="206" font-family="JetBrains Mono" font-size="9" fill="#92400E" text-anchor="middle">session-bbb ✓</text>
...
<text x="465" y="206" font-family="JetBrains Mono" font-size="9" fill="#1A1A1A" text-anchor="middle">session-ccc</text>
...
<text x="312" y="263" font-family="serif" font-size="9" font-style="italic" fill="#6B6258">matched session</text>
...
<text x="300" y="392" font-family="serif" font-size="9" font-style="italic" fill="#6B6258" text-anchor="middle">summary, not the raw matches</text>
...
The tool stack and the backend list are what the closed loop sits on. Skills run inside one of the seven backends. Subagents run inside a (possibly different) one of the seven. The agent's adaptation surface scales with what the backends allow, and the backends' diversity is the deployment knob.
...
<figcaption><strong>Fig 13.</strong> All six frameworks on the same map. None sits at the v4 corner with all three dials at the top rung; each picks its own placement. OpenClaw anchors the gateway rung; Hermes anchors the skill-creation rung. The four Claw variants scatter, and four of their distinctive concerns (container minimalism, hardware portability, deploy-anywhere, feature breadth) sit off the map entirely. The dial map captures what it can; the off-axis annotations close the rest.</figcaption>
</figure>
...
- [breakcafe/picoclaw](https://github.com/breakcafe/picoclaw). PicoClaw (the name-collision serverless fork of NanoClaw, distinct project). package.json v1.2.23 (no GitHub release), accessed 2026-05-08.
