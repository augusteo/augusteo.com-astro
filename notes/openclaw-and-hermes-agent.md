...
**Star count milestone.** Star History primary post (`https://www.star-history.com/blog/openclaw-surpasses-react-most-starred-software/`, 2026-03-01): "OpenClaw has now crossed 250K+ stars, overtaking React to become the most-starred non-aggregator software project on GitHub." The qualifier "non-aggregator software project" is load-bearing — GitHub's overall most-starred repos include awesome-list aggregators in the millions (freeCodeCamp, EbookFoundation/free-programming-books, sindresorhus/awesome). OpenClaw passed React, the previous non-aggregator leader, at ~243K stars. **Live count via `gh api repos/openclaw/openclaw` on 2026-05-08: 369,860.**
...
**agentskills.io.** Hermes README: "Compatible with the agentskills.io open standard." agentskills.io is a real published spec (`https://agentskills.io/specification`), originally developed by Anthropic, released as an open standard, and adopted by 30+ products including Claude Code, Cursor, GitHub Copilot, Goose, OpenHands, Letta, Roo Code. **Implication for the post:** Hermes's `agentskills.io` compatibility is broad-industry table stakes, not a Hermes-specific moat.
...
**Install.** README Quick Install: `curl -fsSL https://raw.githubusercontent.com/NousResearch/hermes-agent/main/scripts/install.sh | bash`. Docs add: "The installer handles **everything**: `uv`, Python 3.11, Node.js 22, `ripgrep`, `ffmpeg`." Platforms: Linux / macOS / WSL2 / Android via Termux / **native Windows (added in v0.13.0 via PowerShell `irm | iex`** — commit `b7fe7ed7bd` "feat(windows-install): bundle portable MinGit instead of relying on winget", 2026-05-07).
...
| 14 | OpenClaw passed React's star count to become the most-starred **non-aggregator software project** on GitHub by 2026-03-01 (NOT "most-starred repo on GitHub overall" — aggregator repos like freeCodeCamp sit higher) | "OpenClaw has now crossed 250K+ stars, overtaking React to become the most-starred non-aggregator software project on GitHub." | star-history.com/blog/openclaw-surpasses-react-most-starred-software/ (2026-03-01) | passes |
...
| 17 | OpenClaw sandbox workspace-access modes: `none` (isolated workspace under `~/.openclaw/sandboxes`), `ro` (mount workspace read-only at `/agent`), `rw` (read-write at `/workspace`) | docs.openclaw.ai/gateway/sandboxing access-mode table | docs.openclaw.ai/gateway/sandboxing accessed 2026-05-08 | passes |
...
| 19 | NanoClaw operates on a session-plus-cron model (per-session DBs `inbound.db`, `outbound.db`; "recurring jobs that run Claude and can message you back") — NOT continuous-lifespan-with-high-agency | github.com/qwibitai/nanoclaw README architecture section | github.com/qwibitai/nanoclaw README accessed 2026-05-08 | passes |
...
| 24 | The peterwoods.online "Claw family taxonomy" is contradicted by primary sources on 3 of 4 projects (NanoClaw, ZeroClaw, ZeptoClaw); PicoClaw is partially-accurate | Per the verdict tables in Sub-topic C of Research notes; each contradiction quoted from each project's own README. peterwoods.online: secondary, "framing under test." | peterwoods.online/blog/the-claw-ai-agent-family (2026-02-19) treated as secondary; primaries listed in rows 18-23 | passes |
| 25 | The four Claw projects are NOT a coordinated family — distinct authors (qwibitai / sipeed / zeroclaw-labs / qhkm), distinct languages (TS / Go / Rust / Rust), no mutual cross-references in their READMEs | Per the four READMEs walked. ZeroClaw README: "other repositories claiming affiliation are unauthorized." ZeptoClaw COMPARISON.md omits ZeroClaw entirely. | github.com/qwibitai/nanoclaw, sipeed/picoclaw, zeroclaw-labs/zeroclaw, qhkm/zeptoclaw READMEs accessed 2026-05-08 | passes |
| 26 | ZeptoClaw is part of "Zepto Stack" (ZeptoPM orchestrator, ZeptoCapsule sandboxer, ZeptoRT durable runtime) — a real coordinated sub-family, separate from "Claw" | qhkm/zeptoclaw README + COMPARISON.md | github.com/qhkm/zeptoclaw README v0.9.2 (2026-04-07) | passes |
...
| 31 | Hermes is MIT-licensed for its own code, but Honcho dependency is AGPL-3.0 | Hermes LICENSE = MIT; Honcho repo `license.spdx_id` = AGPL-3.0 | github.com/NousResearch/hermes-agent/blob/v2026.5.7/LICENSE; gh API repos/plastic-labs/honcho | passes |
...
| 37 | Hermes's Skills Hub is compatible with the agentskills.io open standard (originally Anthropic-developed; broadly adopted across 30+ AI tools) | Hermes README: "Compatible with the agentskills.io open standard." agentskills.io: "The Agent Skills format was originally developed by Anthropic, released as an open standard, and has been adopted by a growing number of agent products." | github.com/NousResearch/hermes-agent/blob/v2026.5.7/README.md; agentskills.io/specification accessed 2026-05-08 | passes |
...
| 39 | Hermes installs via single curl command on Linux / macOS / WSL2 / Android (Termux); native Windows install added in v0.13.0 | README Quick Install: `curl -fsSL https://raw.githubusercontent.com/NousResearch/hermes-agent/main/scripts/install.sh \| bash`. Docs Installation page: "The installer handles **everything**: `uv`, Python 3.11, Node.js 22, `ripgrep`, `ffmpeg`." Windows commit `b7fe7ed7bd` "feat(windows-install): bundle portable MinGit instead of relying on winget" 2026-05-07. | github.com/NousResearch/hermes-agent/blob/v2026.5.7/README.md; hermes-agent.nousresearch.com/docs/getting-started/installation; commit b7fe7ed7bd | passes |
...
**Marginal-row closure** (per `research-protocol.md` "Marginal-source closure rule"):
...
No other marginal rows. No stale rows. No fabricated quotes (every quote was returned by a research subagent against a real URL). No misattributed sources (the "self-frames as" rows are explicitly framed as quoted self-description). No unannotated stale rows.
