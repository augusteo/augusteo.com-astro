...
**Run 1 (2026-05-09).** Codex consult fired against the full MDX + notes file (Spec / Throughline / Research notes / Claim-source matrix / Related posts / Phase 7 freshness pass / all prior Codex sections). Output 470 KB; the full transcript is preserved at [`notes/openclaw-and-hermes-agent-codex-final-20260509.md`](openclaw-and-hermes-agent-codex-final-20260509.md). Findings: **7 STRUCTURAL + 1 COSMETIC**. No freshness regression. Related-posts cross-link rules pass.

The seven STRUCTURAL findings cluster around Act 4 (Hermes) and a single Act-1 claim:

| # | Label | Location | Fix shape |
|---|---|---|---|
| F1 | Hermes placed on Gateway/control-plane surface dial without a matrix row | index.mdx §18 (lines 1119, 1121) | Either add matrix row backed by Hermes README's CLI/server architecture, or soften the "same as OpenClaw's" placement language so it doesn't imply identical control-plane semantics |
| F2 | Hermes sessions described as "training data" — wrong mental model (implies fine-tuning) | index.mdx §11 transition (line 738) | Replace "training data" with something rooted in row 28: "experience the agent indexes, summarizes, and turns into named skills" |
| F3 | FTS5 section over-specifies storage mechanism: SQLite + JSONL + top-K + raw-exclusion are inferred, not in row 28 | index.mdx §14 + Fig 9 captions (line 835) | Pull prose + Fig 9 caption back to row 28's verbatim ("FTS5 session search with LLM summarization") + clearly mark SQLite/JSONL/top-K as inference if kept |
| F4 | Skill creation drifts past row 28: "user judged successful," "named procedure with parameters," "steps that produced the answer" | index.mdx §13 (line 819) | Reduce to row 28 verbatim ("autonomous skill creation after complex tasks; skills self-improve during use"); strip the parameterization/user-judgment overlay |
| F5 | Honcho licensing adds unsupported deployment-frequency claim ("for most personal-assistant deployments") | index.mdx §15 (line 914) | Replace with row 31's actual scope: install paths and what they pull, no claims about deployment frequency |
| F6 | Migration-tooling sentence infers Nous's intent from tooling existence (row 42 explicitly blocks the wave framing) | index.mdx §11 (line 720) | Drop the "expected enough migration traffic" inference; keep only row 41's documented tooling description |
| F7 | New SQLite/FTS5 docs source linked in prose but not in matrix or References | index.mdx §14 (line 835) | Either add a matrix row and References entry, or remove the inline link and revert to row 28 |
| C1 | Inline named-source mentions in §16 ("the official docs page", "The README at v2026.5.7") not hyperlinked | index.mdx §16 (lines 997, 999) | Wrap the named phrases in markdown links to the same URLs used in `## References` |

Findings: 7 STRUCTURAL, 1 COSMETIC.

[full findings: notes/openclaw-and-hermes-agent-codex-final-20260509.md]

**Run 2 (2026-05-09).** Verification pass after Run 1 fixes were committed. Codex confirmed F2–F7 + C1 hold but caught **F1 overcorrected**: my Run 1 fix had moved Hermes's surface-dial placement from gateway to CLI/API. Codex spot-checked the Hermes v2026.5.7 README and surfaced explicit gateway-architecture quotes — "Telegram, Discord, Slack, WhatsApp, Signal, and CLI — all from a single gateway process" (README line 21); "Hermes has two entry points: start the terminal UI with `hermes`, or run the gateway and talk to it from Telegram/Discord/Slack/WhatsApp/Signal/Email" (line 70). The README documents `hermes gateway` as a documented entry point; the architectural placement at gateway is correct, the matrix just lacked a row for it.

| # | Label | Resolution |
|---|---|---|
| RF1 | F1 overcorrection: Hermes is in fact at gateway per its README (`hermes gateway`); Run 1 moved it to CLI/API in error | Added matrix row 33b backing Hermes-as-gateway with README quotes; moved Hermes marker on Fig 13 back to gateway (y=216); restored §17 to "both at gateway" framing with architectural-weight nuance (OpenClaw IS the gateway; Hermes ships gateway as one of two entry points alongside the CLI). §18 restored to "share two dials, split decisively on the third." |

Findings: 1 STRUCTURAL, 0 COSMETIC. F2–F7 + C1 confirmed holding.
