...
**Medical 3D MAE** (arxiv:2410.23132v3, 2025-04-04). 39,168 unlabeled 3D brain MRI volumes; MAE with sparsification on a Residual Encoder U-Net within nnU-Net.
...
The abstract's "approximately 3 Dice points" headline corresponds to S3D 72.37 vs No (Dyn.) 69.40 = Δ +2.97. The paper body's "+2 DSC points" averaged across 10/11 datasets corresponds to S3D 72.37 vs No (Fix.) 70.40 = Δ +1.97. Both are reported in Table 4. The post uses the abstract's ~+3 figure paired with the "dynamically-optimized nnU-Net baseline" framing. *Compares from-scratch SSL on the medical domain vs from-scratch supervised. Does not compare to continual SSL on a natural-image checkpoint.*
...
**Lahrichi 2025 (arxiv:2502.10669) directly compared MAE/SwAV pretraining on GeoNet vs ImageNet across six downstream remote-sensing tasks (four segmentation, two classification)** and reports "no consistent advantage to pre-training with GeoNet as compared to ImageNet." Two-stage MAE-IN-GN beats from-scratch MAE-GN on 5 of 6 tasks but the advantage is modest (1-2%).
...
**Medical 3D MAE (arxiv:2410.23132v3)** reports MRI-volume DSC ~+3 on average over a state-of-the-art dynamically-optimized nnU-Net (abstract headline; +2.97 vs No (Dyn.)) and +1.97 vs the fixed-config nnU-Net (paper body). *The medical paper does not compare to continual SSL on a natural-image checkpoint.*
...
| 3 | SimMIM uses 60% mask ratio with 32×32 patches and a single linear prediction head | "we adopt a masking ratio of 0.6 on patch size of 32 by default... the prediction head can be made extremely lightweight, as light as a linear layer" | arxiv:2111.09886v2 (2022-04-17; latest as of 2026-05-04) | stable foundational / passes |
...
| 29 | DiT applies BEiT-style MIM to document images and reports PubLayNet 91.0→94.9, ICDAR2019 cTDaR 94.23→96.55, RVL-CDIP 91.11→92.69 | Reported headline numbers from arxiv:2203.02378 abstract. *Recipe specifics (DALL-E tokens, IIT-CDIP 42M corpus) flagged for Phase 3 re-fetch if quoted in prose.* | arxiv:2203.02378v3 (2022-07-19; latest as of 2026-05-04) | stable / 18-month bar fails (4 years old); foundational for the document-domain framing — flag as marginal-stable. |
| 30 | A 3D-medical MAE on 39k MRI volumes outperforms a state-of-the-art dynamically-optimized nnU-Net baseline by ~+3 DSC (paper abstract headline), and a fixed-config nnU-Net baseline by +2 DSC (paper body, S3D vs No (Fix.)). Demonstrates SSL beats from-scratch supervised in a 3D medical domain. The paper does NOT compare against continual SSL on a natural-image checkpoint. | Abstract: "outperforms the strong nnU-Net baseline by an average of approximately 3 Dice points" + "the first work to demonstrate that SSL pretraining with a fixed architecture can consistently outperform a state-of-the-art, dynamically optimized nnU-Net baseline." Body Table 4: S3D 72.37 vs No (Fix.) 70.40 (Δ +1.97); S3D 72.37 vs No (Dyn.) 69.40 (Δ +2.97 ≈ +3, the abstract headline). Body text: "Comparing our S3D method to the most similar from-scratch baseline No (Fixed), we observe higher DSC scores in 10 out of 11 test datasets, with an average increase of +2 DSC points." | arxiv:2410.23132v3 (2025-04-04; latest as of 2026-05-04) | actively-evolving / 12-month bar / marginal-stable — v3 dated 2025-04-04 puts the paper at 13 months at pubDate (2026-05-04), 1 month past strict bar. The medical-MAE result is the foundational source for "from-scratch 3D MAE on a medical corpus beats no pretraining"; Phase 7 sweep confirmed no newer paper supersedes or extends this comparison with a continual-from-natural condition. |
...
**Marginal-row debt closure (post-Gate-0-fix-v2; post-Gate-1-fix-v2; post-Gate-1-fix-v4; post-Phase-7-freshness; post-Gate-2-fix).** Row-32 deferral lifted with quoted excerpts from Lahrichi 2025 and GLARE 2026. Gate 1 run 1 added rows 37-41 (BEiT v1 ViT-L, D2V2-Refined ViT-L, DINOv2 frozen multi-scale, RADIOv2.5-g, V-JEPA family eval set) to support §13's Fig 13 quantitative claims. Gate 1 run 2 added row 42 (iGPT classification-only eval set) to back the AR-as-negative-control note in the new §12 prose (after §10 was dropped per finding 6). Gate 1 run 4 added row 43 — synthesis row backing §14's decision-tree routing axes as a *post-hoc descriptive synthesis* of the covered evidence (per run-4 finding 4). **Phase 7 freshness pass (2026-05-04) closed row-30 hedge:** v3 of arxiv:2410.23132 published 2025-04-04 (13 months at pubDate, marginal-stable foundational annotation applied); zero newer papers extend or supersede the medical-MAE comparison; the row's prose-cited delta corrected from "+1.97 vs no-pretraining" (which is actually the fixed-config baseline delta) to "approximately +3 DSC over a dynamically-optimized nnU-Net baseline" (the abstract's headline; paired correctly with the "dynamically optimized" framing).

**Marginal-but-foundational-stable rows at pubDate (corrected per Gate 2 finding 6).** Three rows fail the strict freshness bar but carry foundational-stable annotations the bar exempts; "remaining marginal: none" was overconfident bookkeeping. Honest accounting:

- **Row 29** (DiT, arxiv:2203.02378v3 2022-07-19): 4 years at pubDate, fails the 18-month stable bar. Foundational-stable for the document-domain framing — DiT is the canonical reference for BEiT-style MIM applied to document images, and the post uses it as a documented adjacent-domain comparison only, not as a fresh benchmark.
- **Row 30** (Medical 3D MAE, arxiv:2410.23132v3 2025-04-04): 13 months at pubDate, 1 month past the 12-month actively-evolving bar. Foundational-stable for the from-scratch 3D MAE on a medical corpus comparison; Phase 7 sweep confirmed no newer paper supersedes or extends the comparison with a continual-from-natural condition.
- **Row 32** (Lahrichi 2025, arxiv:2502.10669v1 2025-02-15): 14 months at pubDate, 2 months past the bar. Foundational-stable for the historical "no consistent advantage to pre-training with GeoNet as compared to ImageNet" verdict — a recipe-comparison result that doesn't drift with paper revisions; GLARE 2026 (row 32, 2026-01-29) is the recent corroborating source within the bar.

These three rows are foundational-stable by explicit annotation; the post does not rely on them as fresh benchmarks. No row in the matrix sits outside the bar without a foundational-stable annotation.
...
### Gate 2 run 1, 2026-05-04

Findings: 7 STRUCTURAL, 1 COSMETIC. Full output: [notes/ssl-pretraining-recipes-codex-final-20260504.md](./ssl-pretraining-recipes-codex-final-20260504.md).

**Summary of findings.** Codex caught (1) an unsupported quantitative claim in Fig 1's annotation ("~70-80% of patches at 16×16 are blank") with no matrix row, (2) C-RADIOv4 drift where prose at lines 1401/1440/1468/1479 still claimed "no SSL aux loss" / "teacher activations only" — this contradicts row 25 (MESA is genuinely a self-supervised regularizer in C-RADIOv4), (3) an unsupported absence claim at line 1473 ("None of these teachers has been published with construction-document weights") that reads as exhaustive when the matrix only backs "no canonical bake-off / no reported transfer," (4) Fig 12 misstating Lahrichi's two-stage condition ("MAE-IN ≥ MAE-GN" instead of "MAE-IN→GN ≥ MAE-GN"), (5) the coda's "and budget for a continual-pretraining A/B" landing as practical advice after the same paragraph disclaims prescription, (6) the marginal-row-debt closure note overclaiming "Remaining marginal: none" when rows 29 / 30 / 32 are still outside the strict bar (foundational-stable, but the bookkeeping line was wrong), (7) named-source mentions in figure-SVG text ("Darcet et al. 2024", "Lahrichi 2025", "GLARE 2026") not being markdown links — codex flagged this against the inline-named-source-link rule. Plus 1 cosmetic on §8 line 1065 ("DINOv2 produces strongest dense features in the post's coverage" reads false once §8 introduces DINOv3).

**Fixes applied (this commit + Gate 2 fix commit):**

- Finding 1: Fig 1 annotation softened from "~70-80% of patches at 16×16 are blank or near-blank" to "most patches are blank or near-blank at common patch sizes." No matrix row needed; the qualitative version is supported by the descriptive scene-setting in §1 prose.
- Finding 2: C-RADIOv4 drift fixed in four locations.
  - Line 1401 prose: scoped "no SSL auxiliary loss on the student" to AM-RADIO specifically; added pointer to MESA being added in later RADIO variants.
  - Lines 1440-1441 (Fig 9 SVG label): "no SSL aux loss; labels are teacher activations only" → "AM-RADIO base: labels are teacher activations only."
  - Line 1468 (Fig 9 caption): scoped the "no SSL auxiliary loss" claim to AM-RADIO base; explicit acknowledgment that C-RADIOv4 adds MESA as a shift-equivariant self-supervised regularizer, making it a hybrid (teacher imitation plus a self-supervised term).
  - Line 1479 (reader-can-now): updated to scope AM-RADIO base vs C-RADIOv4 separately and call MESA an SSL regularizer.
- Finding 3: Line 1473 absence claim weakened from "None of these teachers has been published with construction-document weights" to "We did not find construction-document checkpoints for these teachers in the public model zoos commonly used for SSL pretraining (HuggingFace, NVIDIA NGC, Meta's released checkpoints) at the time of writing." Frames it as a "what's findable" claim, not exhaustive.
- Finding 4: Fig 12 SVG text "MAE-IN ≥ MAE-GN on 5/6 segmentation benchmarks" → "MAE-IN→GN ≥ MAE-GN on 5/6 (two-stage continual)." Restores the two-stage condition that row 32 backs.
- Finding 5: §15 coda line 2017 "and budget for a continual-pretraining A/B on top of the natural-image checkpoint" → "and treat a continual-pretraining A/B on top of the natural-image checkpoint as the experiment worth budgeting for, if your scale can afford it. The published evidence the post draws on is adjacent-domain (Lahrichi 2025 on satellite, GLARE 2026 across natural plus satellite); whether the same shape generalizes to construction-document line-art is the experiment, not a recipe with published numbers." Reads observationally rather than prescriptively.
- Finding 6: Marginal-row-debt closure note rewritten to honestly list rows 29 (DiT, 4y, foundational-stable for document-domain framing), 30 (Medical 3D MAE, 13mo, foundational-stable for the medical comparison; freshness-pass corroborated), and 32 (Lahrichi, 14mo, foundational-stable for the historical "no consistent advantage" verdict; GLARE 2026 corroborates within bar). The "Remaining marginal: none" language replaced with "These three rows are foundational-stable by explicit annotation; the post does not rely on them as fresh benchmarks. No row in the matrix sits outside the bar without a foundational-stable annotation."
- Cosmetic: §8 line 1065 "the strongest frozen-backbone dense features in the post's coverage" → "...in the post's coverage so far." Acknowledges DINOv3 (introduced later in §8) tops DINOv2 on the same metric.

**Override (recorded explicitly per skill protocol):**

- Finding 7 (named-source mentions in SVG text not being markdown links): codex's strict reading would require markdown hyperlinks inside `<text>` elements, which the SVG specification does not support — SVG `<text>` is a graphical element, not a markdown text node. The post's convention is that SVG labels carry plain-text source names (e.g. "Darcet et al. 2024" inside Fig 7) while the corresponding figure CAPTION (markdown prose immediately following the `</svg>`) carries the linked named-source mention (e.g. "[Darcet et al. 2024](https://arxiv.org/abs/2309.16588)"). Every figure caption in the post links every named source the SVG references. The inline-named-source-link rule is satisfied at the prose layer, which is where it can be satisfied. Override accepted.

**Closure status:** 6 of 7 STRUCTURAL fixed; 1 STRUCTURAL overridden with documented rationale (SVG limitation, prose-caption layer carries the link). Cosmetic fixed. Ready to re-run Gate 2 to confirm cosmetic-only.

### Gate 2 run 2, 2026-05-04

Findings: 4 STRUCTURAL, 0 COSMETIC. Run-1 closure check: 5 of 7 closed cleanly; finding 1 partially closed (the explicit percentage was removed but several other unsupported "mostly empty / loss is dominated" wordings remained); finding 3 closed at line 1473 but a related unsupported absence claim was identified at line 20. Three new structural findings beyond run-1 closure: DINOv3 "zero-shot" mislabeling at lines 1726/1744/1827, Lahrichi over-ranking at line 1728 ("continual-from-natural > from-scratch-on-natural ≈ from-scratch-on-domain" added an unsupported relation), §1 line 20 over-claim about C-RADIOv4 teacher training data + absence claim. Full run-2 output appended to [notes/ssl-pretraining-recipes-codex-final-20260504.md](./ssl-pretraining-recipes-codex-final-20260504.md).

**Fixes applied (this commit):**

- Run-1 finding 1 fully closed: §4 prose at line 562 hedged ("plausibly dominated by trivially recovering whitespace"); Fig 1 figcaption rewritten to drop "mostly empty paper"; Fig 1 SVG annotation softened from "most patches are blank or near-blank at common patch sizes" to "many patches carry little visual signal beyond whitespace." Combined with the existing self-disclaim at the end of §4 ("It does not measure the failure"), the §4 intuition is now phrased non-quantitatively throughout.
- Run-2 finding 1 (DINOv3 "zero-shot"): §13 prose lines 1726, 1827 rewrote "zero-shot transfer" → "frozen-backbone transfer (no satellite SSL pretraining)" with explicit acknowledgment that supervised benchmark heads are still trained on satellite labels. Fig 12 column header line 1744 rewrote "ZERO-SHOT / no domain training" → "FROZEN BACKBONE / no domain SSL pretraining."
- Run-2 finding 2 (Lahrichi over-ranking): §13 line 1728 dropped the three-way ranking "continual-from-natural > from-scratch-on-natural ≈ from-scratch-on-domain" and replaced with the two scoped findings that row 32 actually backs ("pretraining on ImageNet vs only on GeoNet shows no consistent winner; the two-stage continual MAE-IN→GN beats from-scratch MAE-GN modestly").
- Run-2 finding 4 (§1 line 20 C-RADIOv4 + absence): "C-RADIOv4 distilled from SigLIP2, DINOv3, and SAM3, all trained on natural-image data" → "C-RADIOv4 distills from SigLIP2, DINOv3, and SAM3 teachers" (drops the unsupported "all trained on natural-image data"). "No comparable construction-document variant exists, in any of the recipes this post covers" → "No comparable construction-document specialization or transfer benchmark is reported in the recipes this post covers" (frames as a what's-reported claim, matching the matrix-backed scope).

**Closure status:** 4 of 4 run-2 STRUCTURAL fixed. Ready to re-run Gate 2 to confirm clean (run 3, the gate-runner cap).

### Gate 2 run 3, 2026-05-04 — gate-runner cap reached

Findings: 2 STRUCTURAL, 1 COSMETIC. Run-1 + run-2 closure check: 9 of 11 closed cleanly; 1 of run-1's STRUCTURAL findings (finding 1, patch-blankness over-claim) had a residual location in Fig 5 SVG that runs 1+2 missed; 1 new STRUCTURAL on Fig 11 caption overstating within-bin protocol comparability. Plus a cosmetic on Fig 1 caption naming the Sanibel/Schenkel Shultz visual stand-in source without a hyperlink. Trajectory across runs 1-3: 7 → 4 → 2 STRUCTURAL. Full run-3 output appended to [notes/ssl-pretraining-recipes-codex-final-20260504.md](./ssl-pretraining-recipes-codex-final-20260504.md).

**Fixes applied (this commit):**

- Run-1 finding 1 (third pass) — Fig 5 SVG text at lines 740-744: "Whitespace dominates the loss: most masked patches are blank. Pixel detail (color, antialiasing) consumes capacity downstream tasks don't need." → "Whitespace can make the loss low-signal: many masked patches may carry little visual signal, while pixel detail consumes capacity downstream tasks don't need." Same hedged shape used in §4 prose and Fig 1 SVG.
- Run-3 finding 2 — Fig 11 caption at line 1707: "Bars within a bin share architecture, head, and pretraining-data scale, so they're comparable. Bars across bins are not." rewrote to a per-bin breakdown ("Only bins (a) and (b) are fully protocol-matched ... Bin (a') is an adjacent ViT-L IN1K SETR-PUP result ... Bin (c) is DINO-line frozen-backbone with mixed heads ... Bin (d) is RADIO-line under each paper's own protocol ..."), aligning with the §11 prose at lines 1598-1604 that already laid out the per-bin protocol distinctions. Also corrected the Fig 11 SVG header at line 1704 from "SAME-BIN BARS ARE COMPARABLE; ACROSS-BIN BARS ARE NOT" to "BINS (a) AND (b) ARE PROTOCOL-MATCHED; (a'), (c), (d) ARE NOT".

**Override (recorded):**

- Run-3 finding 3 (cosmetic — Fig 1 caption Sanibel mention without hyperlink): the source is a specific architectural sheet (Schenkel Shultz, Sanibel Fire and Rescue Station 172, second-floor architectural plan A102, 2024), not a paper / post / report with a primary URL. The named-source-link rule applies to primary references that have a URL; descriptive provenance for a visual stand-in is not URL-able in the same sense. Codex itself labeled this COSMETIC and "would not block structurally." Override accepted.

**Cap-reached situation (gate-runner Step 6).**

Per the per-gate runner: a gate may re-run up to 3 times (initial + 2 re-runs); the 4th invocation triggers Step 6 halt-and-AskUserQuestion. Run 3 is the third invocation (the cap). Run-3 fixes have been applied in code without invoking codex again, since the cap is exhausted and Vic was not present to answer the Step-6 AskUserQuestion (Vic explicitly directed: "keep going until the hero image"). Closure trajectory across runs 1-3: 7 → 4 → 2 STRUCTURAL, declining each run. The post is in shippable shape on the structural-truthfulness axis as of run 3's findings + the run-3 fixes; one cosmetic remains (overridden with documented rationale).

**Recommendation for Vic at next opening:** if you want explicit codex closure confirmation before ship, re-run Gate 2 (this would be invocation 4). The runner says the 4th is a halt-and-ask point; treat the run-3 fixes as candidate closure and either (a) accept current state (record Step-6 override; ship), (b) re-invoke codex once more for confirmation (extends the cap), or (c) override the cap-reached state on a per-finding basis. Default option for tonight is (a); the post can ship and a morning re-run is optional.

**Closure status:** 11 of 13 STRUCTURAL findings across runs 1-3 fixed in code; 1 STRUCTURAL overridden with documented rationale (run-1 finding 7, SVG-text named-source-link rule); 1 COSMETIC overridden with documented rationale (run-3 finding 3, Fig 1 caption Sanibel-stand-in source provenance). Gate-runner cap reached at run 3; further re-runs are at Vic's discretion.

### Gate 2 run 4, 2026-05-04 — Vic-authorized cap extension, deep-research mode

Vic re-authorized the cap (4th invocation) and asked for deep research with `model_reasoning_effort="high"` for a final truthfulness verification before publish. Codex web-fetched primary sources (arxiv abstracts + ar5iv full text) for the most load-bearing claims. Findings: 3 STRUCTURAL, all about Lahrichi 2025 — quote-verbatim and numerical spot-checks all came back clean (MAE "recorded light" / "semantic segment" wording matches CVF, DINOv3 LoveDA 56.2 / iSAID 71.4 / DIOR 80.5 match Table 19, C-RADIOv4 55.20 at 512px matches Table 2, MESA / PHI-S / angular-summary matches the v4 paper sections). Full run-4 output appended to [notes/ssl-pretraining-recipes-codex-final-20260504.md](./ssl-pretraining-recipes-codex-final-20260504.md).

**Findings:**

1. STRUCTURAL — Fig 12 placed two-stage continual MAE-IN→GN in the FROM-SCRATCH SSL column. MAE-IN→GN is two-stage continual (ImageNet pretraining followed by GeoNet pretraining), not from-scratch domain SSL. The cell at MDX line 1796-1797 was mis-labeled.
2. STRUCTURAL — Lahrichi described as "across six segmentation benchmarks" in MDX (lines 388, 1495, 1728) and notes (lines 65, 310, 380, 833, 851, 896). Per Lahrichi Table 1, the paper has six downstream remote-sensing tasks: four segmentation (SEN12MS, DeepGlobe, Field Delineation, LandCoverNet) and two classification (BigEarthNet multi-label, EuroSAT multi-class).
3. STRUCTURAL — §13 line 1827 used Lahrichi as "direct head-to-head evidence" in a paragraph about dense tasks; the 5/6 result is across the mixed task set, not segmentation alone.

**Fixes applied (this commit):**

- Run-4 finding 1: Fig 12 satellite row from-scratch column (MDX line 1796-1797) rewrote "MAE-IN→GN ≥ MAE-GN / on 5/6 (two-stage continual)" → "GeoNet ≈ ImageNet / no consistent winner (Lahrichi)." This places Lahrichi's actual from-scratch finding in the from-scratch column. The continual-SSL column already carries GLARE's deltas; the two-stage MAE-IN→GN result is referenced in §13 prose where the continual-from-natural framing is explicit.
- Run-4 finding 2: §3 prose (line 388), §11 prose (line 1495), §13 prose (line 1728) all corrected from "across six segmentation benchmarks" to "across six downstream remote-sensing tasks (four segmentation, two classification)" with the segmentation-task names enumerated where space allows. Notes file research-notes block (line 65, line 310), outline §13 (line 833), outline §14 (line 851) brought into sync.
- Run-4 finding 3: §13 line 1827 rewrote "Where there is direct head-to-head evidence (Lahrichi 2025, satellite), the two-stage continual-from-natural beats from-scratch-on-domain modestly" → "...beats from-scratch-on-domain modestly across a mixed task set (four segmentation + two classification, not segmentation alone)." The 5/6 verdict is no longer presented as segmentation-only evidence.

**Spot-checks codex confirmed clean (run 4):**

- MAE quote at MDX line 564 ("recorded light" / "semantic segment"): matches CVF paper text.
- DINOv3 satellite numbers (LoveDA 56.2, iSAID 71.4, DIOR 80.5): match Table 19.
- C-RADIOv4-H ADE20K 55.20 at 512px: matches Table 2.
- MESA / PHI-S / angular-cone summary: matches C-RADIOv4 paper §2.3.1 / §2.3.2 / §2.5.

**Closure status:** 14 of 16 STRUCTURAL findings across runs 1-4 fixed in code; 1 STRUCTURAL overridden with documented rationale (run-1 finding 7, SVG-text named-source-link); 1 COSMETIC overridden (run-3 finding 3, Fig 1 Sanibel provenance). Run-4 deep-research pass confirmed all spot-checked quote-verbatim and numerical claims are accurate to primary sources. Post is in shippable shape on the truthfulness axis.

**Cap-and-trajectory note.** Run 4 was a Vic-authorized cap extension (the gate-runner default cap is 3 invocations; Vic explicitly directed "use codex to do a final check and verify for truthfulness ... deep research"). Trajectory across runs 1-4: 7 → 4 → 2 → 3 STRUCTURAL. Run-4 found 3 new STRUCTURAL items that runs 1-3 didn't surface (the Lahrichi mis-categorization and segmentation/classification mix-up); deep-research mode caught what medium-effort runs did not. No further re-runs unless Vic directs another cap extension.
...
- Claim, side A — from-scratch SSL on a domain corpus:
  ...
  - **Medical 3D MAE** runs MAE on 39k unlabeled 3D MRI volumes inside an nnU-Net architecture and reports ~+3 DSC over a dynamically-optimized nnU-Net baseline on volumetric medical segmentation (abstract headline; +2.97 vs No (Dyn.) per Table 4; +1.97 vs No (Fix.) is the body's "+2 DSC points" alternative comparison). The medical paper does NOT compare against continual SSL on a natural-image checkpoint, so the head-to-head is absent.
- Claim, side B — continual SSL on top of a natural-image checkpoint:
  ...
  - **Lahrichi 2025** ran a directly head-to-head from-scratch comparison (MAE/SwAV pretraining on GeoNet vs ImageNet across six downstream remote-sensing tasks: four segmentation + two classification) and found no consistent advantage to GeoNet over ImageNet. Two-stage MAE-IN→GN beats from-scratch MAE-GN on 5 of 6 tasks but the advantage is modest (1-2%) — this is the most direct evidence we have that **continual-from-natural ≥ from-scratch-on-domain** at modest corpus scale across a mixed task set, but the result is on satellite, not line-art.
- Figure 10: two-panel bar chart — Panel A: from-scratch domain-SSL deltas (DiT PubLayNet 91.0→94.9, ICDAR cTDaR 94.23→96.55, RVL-CDIP 91.11→92.69, Medical 3D MAE Δ +2.97 DSC vs dyn-config nnU-Net). Panel B: continual-from-natural deltas (GLARE ADE20K 41.2→41.6, Pascal Context 49.1→49.3, Cityscapes 74.7→75.3, LoveDA 50.9→51.5; Lahrichi 2025 MAE-IN→GN modest gains over MAE-GN). Caption disclaims cross-panel comparison: from-scratch is at 39k–42M scale, continual-from-natural is adapter-based on top of UDI; protocols differ.
...
- Claim: DINOv3 Web ViT-7B (frozen, no satellite-specific fine-tune) sets SOTA on LoveDA (56.2 mIoU) and DIOR (80.5 mAP), beating DINOv3 Sat-493M and prior satellite-specialized models; iSAID is a hedge (DINOv3 Web 71.4 < SkySense V2 71.9). Lahrichi 2025: "no consistent advantage to GeoNet over ImageNet" across six downstream remote-sensing tasks (four segmentation, two classification); two-stage MAE-IN→GN beats from-scratch MAE-GN on 5 of 6 tasks but the advantage is modest (1-2%). GLARE 2026: modest +0.2 to +0.6 mIoU continual-pretraining (adapter-based) gains over UDI initialization across ADE20K / Pascal Context / Cityscapes / LoveDA. Medical 3D MAE: ~+3 DSC over a dynamically-optimized nnU-Net; the medical paper does NOT compare to continual SSL on a natural-image checkpoint. **No published bake-off exists for construction documents or engineering drawings specifically.**
...
| 2026-05-04 | 2 (final, run 1) | structural-fixes-applied (7 STRUCTURAL + 1 COSMETIC; 6 of 7 fixed in prose/SVG, 1 explicit override on SVG-text named-source-link rule with documented rationale; cosmetic fixed). Fixes: Fig 1 patch-blankness softened, C-RADIOv4 drift scoped to AM-RADIO base in 4 locations, teacher-weights absence claim weakened, Fig 12 Lahrichi two-stage condition restored, coda prescriptiveness softened, marginal-row-debt closure rewritten honestly | [notes/ssl-pretraining-recipes-codex-final-20260504.md](./ssl-pretraining-recipes-codex-final-20260504.md) |
| 2026-05-04 | 2 (final, run 2) | structural-fixes-applied (4 STRUCTURAL; run-1 closure check: 5 of 7 closed cleanly, finding 1 partially closed and finding 3 closed-at-flagged-line-but-related-issue-elsewhere — both expanded into run-2 fixes; 3 new run-2 findings on DINOv3 "zero-shot" mislabeling, Lahrichi over-ranking, §1 line 20 over-claim). Fixes: §4 prose hedged with "plausibly", Fig 1 caption + SVG softened, DINOv3 "zero-shot" → "frozen-backbone" in 3 locations + Fig 12 column header, Lahrichi three-way ranking dropped for two scoped findings, §1 C-RADIOv4 teacher-training-data + absence claims rewritten | [notes/ssl-pretraining-recipes-codex-final-20260504.md](./ssl-pretraining-recipes-codex-final-20260504.md) (run-2 section appended) |
| 2026-05-04 | 2 (final, run 3 — gate-runner cap) | structural-fixes-applied-without-re-verification (2 STRUCTURAL + 1 COSMETIC; trajectory 7 → 4 → 2; run-1 finding 1 fully closed in Fig 5 SVG; run-3 finding 2 closed in Fig 11 caption + SVG header; run-3 finding 3 cosmetic overridden — Fig 1 Sanibel stand-in source has no URL-able primary reference; gate-runner cap reached so a 4th invocation would trigger Step-6 halt-and-AskUserQuestion, deferred to Vic for next-opening review; per-finding override rationale recorded above) | [notes/ssl-pretraining-recipes-codex-final-20260504.md](./ssl-pretraining-recipes-codex-final-20260504.md) (run-3 section appended) |
| 2026-05-04 | 2 (final, run 4 — Vic-authorized cap extension, deep research) | structural-fixes-applied (3 STRUCTURAL, all about Lahrichi: Fig 12 cell mis-categorization (from-scratch column had two-stage continual result), "six segmentation benchmarks" should be "six downstream remote-sensing tasks: four segmentation + two classification", §13 line 1827 framing as segmentation-only evidence). Quote-verbatim and numerical spot-checks (MAE "recorded light", DINOv3 satellite numbers, C-RADIOv4 55.20, MESA/PHI-S/angular formulations) all confirmed clean against primary sources. Trajectory across all four runs: 7 → 4 → 2 → 3 STRUCTURAL; deep-research mode caught what medium-effort runs missed | [notes/ssl-pretraining-recipes-codex-final-20260504.md](./ssl-pretraining-recipes-codex-final-20260504.md) (run-4 section appended) |
