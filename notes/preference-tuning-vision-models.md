...
Bradley-Terry (the statistical spine under all of this): P(i beats j) = exp(s_i)/(exp(s_i)+exp(s_j)) = σ(s_i − s_j). Confirmed real reference: Bradley & Terry, "Rank Analysis of Incomplete Block Designs I: The Method of Paired Comparisons," Biometrika 39(3-4):324-345 (1952). Every method that CONVERTS a verdict into a reward reduces a preference to a sigmoid of a score difference; that single equation is what DPO optimizes directly. NOT the spine of the whole post: GRPO, GSPO, SAO and Flow-GRPO are clipped policy-gradient objectives with no Bradley-Terry term. Do not write that the sigmoid is inside 'every loss'.
...
> Recipe (EXACT, re-verified 2026-08-06; the paper is first person, do NOT re-quote it in third person): "We collect training samples (I, M) → (S, T, R, t, s) and preference data from humans and use them in both supervised finetuning (SFT) and direct preference optimization (DPO)"; "This alignment (step 2) can be repeated, first collecting data with the current model and then improving the model with the new data."; "This creates a virtuous cycle with humans providing the supervision."
...
> uses "multimodal LLMs as 'AI annotators'" and fine-tunes MLLMs into "effective 'AI verifiers' that achieve near-human accuracy"; "By delegating certain tasks to AI annotators, models that match or surpass human accuracy, we more than double the throughput compared to a human-only annotation pipeline." (EXACT, re-verified 2026-08-06 against the SAM 3 body; "the throughput is more than doubled" is NOT the paper's wording.)
...
CRITICAL FRAMING FOR THE POST: SAO is the counter-swing to §5 of the post. §5 celebrated GRPO *dropping the critic*. SAO argues that for single-rollout async agentic RL you must *bring the value-model critic back* (and make it TRAIN RELIABLY). Do NOT call it "cheap": K=2 is MORE critic compute, Frozen-Attention is a stability measure, and the paper itself says a value function "essentially doubl[es] the memory footprint during training." This is the honest "the pendulum swings back" beat — exactly the kind of counterweight the post already values (§6). The critic never was strictly obsolete; GRPO's group baseline replaced it only when you can afford a group.
...
> Table 1 also lists two SAO-section ablation rows BELOW the SAO row: SAO (w/ DIS only) AIME2025 94.2 / BeyondAIME 71.5, and **GRPO (+ DIS) AIME2025 93.5 / BeyondAIME 70.8 / HMMT 84.0 / IMOAnswerBench 70.0**. CRITICAL: 93.5, not 84.2, is the STRONGEST GRPO baseline on AIME2025. The 84.2 row is "GRPO (w/ python)", i.e. standard GRPO, and per §4.2 the scores for collapsing models "represent their final valid performance before collapsing." Any sentence framing a GRPO comparison as "the strongest GRPO baseline" must use 93.5.
> Table 2 (SWE-Bench Verified, Accuracy %): Qwen3-30B-A3B 23.0; +GRPO (w/ DIS) 27.0; +SAO (ours) 29.8. (Here 27.0 IS the only and strongest GRPO row, so 29.8 vs 27.0 is a valid strongest-baseline comparison.)
...
**Phase 7 freshness re-check (originally 2026-06-05):** all sources were fetched that day. One targeted re-check: SAM 3D (rows 18-23) bumped to **v2 (2026-06-02)** — verified the three load-bearing claims (human- and model-in-the-loop pipeline, breaking the 3D "data barrier", "at least a 5:1 win rate") are UNCHANGED from v1.

**pubDate correction (2026-08-06):** pubDate was 2026-06-05, which was impossible once §12 was added on 2026-07-10 citing SAO (arxiv:2607.07508, submitted 2026-07-08) — the post claimed to predate a paper it cites. pubDate is now 2026-08-06. Rows 37-43 (SAO) were researched 2026-07-10 and re-verified against the PDF 2026-08-06; every other row was fetched 2026-06-05 / 2026-06-06 and spot-re-verified 2026-08-06 (see the truthfulness-pass row in Codex history). **If Vic ships later than 2026-08-06, bump pubDate again and re-check rows 37-43 for a newer SAO version.**
...
| 8 | DPO eliminates the reward model and the RL sampling loop by mapping reward→policy analytically, leaving a simple classification loss. | Current arxiv abstract: "computationally lightweight, eliminating the need for sampling from the LM during fine-tuning or performing significant hyperparameter tuning."; Introduction: "we show how to directly optimize a language model to adhere to human preferences, without explicit reward modeling or reinforcement learning." | arxiv:2305.18290 (2023-05-29) | foundational/canonical (the topic's core method) / passes |
<!-- 2026-08-06 quote-version note: an earlier revision of this row quoted "eliminating the need for fitting a reward model, sampling from the LM during fine-tuning, or performing significant hyperparameter tuning" (the NeurIPS camera-ready wording). The live arxiv abstract omits "fitting a reward model,". §4 of the MDX quotes the live arxiv wording plus the Introduction sentence; both re-verified verbatim 2026-08-06. Do not "restore" the camera-ready string into the MDX. -->
...
| 19 | SAM 3D's real-world alignment uses SFT + DPO on human preference data, repeated as a virtuous cycle (the data engine). | EXACT sentences (re-verified 2026-08-06 against the paper body): "We collect training samples (I, M) → (S, T, R, t, s) and preference data from humans and use them in both supervised finetuning (SFT) and direct preference optimization (DPO)"; "This alignment (step 2) can be repeated, first collecting data with the current model and then improving the model with the new data."; "This creates a virtuous cycle with humans providing the supervision." | arxiv:2511.16624 (2025-11-20) | actively-evolving / 12-mo / passes |
<!-- 2026-08-06 quote-fidelity fix: §10 previously rendered the first sentence as a third-person quote ("SAM 3D 'collects training samples and preference data from humans and uses them in both...'") and quoted "this alignment can be repeated" without the "(step 2)". Neither was verbatim. §10 now paraphrases the frame and quotes only the exact fragments. The paper is first-person plural; any future quotation must either keep "We collect... and use them..." or stay outside the quote marks. -->
| 20 | SAM 3D's data engine scales because annotators verify/rank/rate model-generated meshes rather than author 3D ground truth, with the hardest cases still going to expert 3D artists. | FULL sentence (re-verified 2026-08-06): "However, our key insight is that verifying or ranking meshes is a more accessible skill."; "We can thus scale by building a data engine asking annotators to rate multiple options generated by a suite of models in the loop, while routing the hardest examples to expert 3D artists to fill data blindspots." | Meta SAM 3D blog https://ai.meta.com/blog/sam-3d/ (first-party, accessed 2026-06-05, re-checked 2026-08-06) | actively-evolving / 12-mo / passes (first-party blog, not arxiv abstract) |
<!-- 2026-08-06 overclaim fix: the Fig 9 caption said annotators are asked "never to build one". The blog's own sentence continues "while routing the hardest examples to expert 3D artists to fill data blindspots", so annotators-never-author is the bet for the bulk, not the whole pipeline. Caption now says "rather than build one ... with only the hardest cases routed to expert 3D artists." Do not restore an absolute "never". -->
...
| 28 | The model-in-the-loop data engine is now a named multi-generation pattern: SAM 3 uses MLLMs as "AI annotators" plus "AI verifiers" at near-human accuracy, more than doubling annotation throughput. | "multimodal LLMs as 'AI annotators'"; "effective 'AI verifiers' that achieve near-human accuracy"; "By delegating certain tasks to AI annotators—models that match or surpass human accuracy—we more than double the throughput compared to a human-only annotation pipeline." (EXACT, re-verified 2026-08-06; an earlier revision of this row paraphrased it as "the throughput is more than doubled", which is not in the paper. §9 of the MDX now states the throughput claim unquoted.) | arxiv:2511.16719 (2025-11) | actively-evolving / 12-mo / passes |
...
| 33 | DAPO is GRPO plus four fixes (clip-higher, dynamic sampling, token-level loss, soft length penalty). NOTE: do NOT call it the "most-reproduced" successor; the paper makes no comparative claim and nothing sourced supports it. | "achieves 50 points on AIME 2024 using Qwen2.5-32B base model." | arxiv:2503.14476 (2025-03-18) | actively-evolving / 12-mo / passes (named as a sibling repair in §11) |
...
| 36 | Pref-GRPO mechanism: GRPO's group-std normalization Â=(r−mean)/std blows up tiny score gaps when std is small ("illusory advantage"); Pref-GRPO replaces the score with each image's pairwise win rate w_i=(1/(G−1))Σ_{j≠i}1(i≻j), which spreads across [0,1] giving real std. | "minimal score differences between images are amplified after normalization, creating illusory advantages that drive the model to over-optimize for trivial gains"; "shifts the optimization objective from score maximization to preference fitting"; "images are pairwise compared within each group using preference RM, and the win rate is used as the reward signal"; "more stable advantages"; "mitigating reward hacking". (Body: reward score rises while quality degrades, "over-saturated colors" / images "appear darker"; win-rate formula w_i=1/(G−1)Σ_{j≠i}1(x₀ⁱ≻x₀ʲ) confirmed exact.) | arxiv:2508.20751 (2025-08-28) | actively-evolving / 12-mo / passes (all quotes re-verified verbatim against abstract + ar5iv body 2026-07-13) |
<!-- 2026-08-06 VERSION DRIFT, IMPORTANT: arxiv:2508.20751 was revised to **v2 on 2026-04-20** and the abstract was substantially rewritten. Every Pref-GRPO quote in rows 29/36 above and in the 2026-07-13 note below is **v1 wording** and is NO LONGER verbatim against the current paper. §13 of the MDX was updated 2026-08-06 to v2 wording:
  v1 "minimal score differences between images are amplified after normalization, creating illusory advantages that drive the model to over-optimize for trivial gains"
  v2 "tightly clustered scores within a group are disproportionately amplified after dividing by the small group standard deviation, driving the policy to over-optimize"
  v1 "shifts the optimization objective from score maximization to preference fitting" -> v2 "reformulates the GRPO objective to pairwise preference fitting"
  v1 "images are pairwise compared within each group using preference RM, and the win rate is used as the reward signal" -> v2 "image pairs within a group are compared by a Pairwise Preference Reward Model (PPRM), and each image's win rate serves as the reward"
  v2 also states the degradation verbatim ("scores keep rising while image quality deteriorates, manifesting as oversaturation or unnaturally dark artifacts"), which the post previously had to paraphrase from the body.
  v2 result quote: "produces more stable advantages than pointwise scoring, substantially alleviates reward hacking, and improves semantic alignment."
  LESSON: an /abs/ fetch can return a cached older abstract. Check the version list on /abs/<id>v1 and diff against /html/<id>v2 before trusting a quote on a live preprint. -->

<!-- 2026-07-13 quote-fidelity fix: four §13 strings previously quoted here were NOT verbatim — "even minor score gaps are disproportionately magnified after normalization" (paper: "amplified", reworded), a fabricated "scores keep rising... oversaturation or unnaturally dark artifacts" composite, a fabricated clipping quote ("cannot remove the underlying tendency to optimize toward unreliable absolute-score increments" — NOT IN PAPER), and "directly suppress[es] the illusory-advantage amplification mechanism" ("suppress" NOT IN PAPER). Replaced in the MDX with verbatim abstract quotes. The word "magnified" and "suppress" do not appear in the paper. -->

| 37 | Asynchronous RL updates the model as rollouts arrive (more efficient for long-horizon agentic tasks than synchronous batch-interleaved RL), but GRPO's group-wise sampling does not fit it: the group must wait for the slowest rollout, and online/agentic settings often give only one trajectory per prompt. | "group-wise sampling in the widely-used GRPO framework does not naturally fit asynchronous agentic training"; "The group-wise sampling induces latency-driven off-policy behavior because the group has to wait for the slower one to finish before fed into training. In addition, group-wise sampling is incompatible with online or complex agentic settings where the environment often provides only a single trajectory feedback per prompt." | arxiv:2607.07508 (2026-07-08) | actively-evolving / 12-mo / passes (fresh; ~1 month old at the 2026-08-06 pubDate) |
...
**8. The same verdict, now on images (Diffusion-DPO).** Bridge the leap in explicit rungs (Gate 1 flagged this as a two-rung jump): (a) DPO's loss needs a likelihood ratio of chosen vs rejected under the policy and the reference (callback to Fig 5); (b) an image generator has no token log-probs to plug in there; (c) but a diffusion/flow model does expose a computable stand-in: the denoising loss, whose negative is the ELBO, a lower bound on the image log-likelihood (NOT a bound on 'probability', and the denoising MSE is the surrogate, not itself the bound); (d) the denoising term takes the place of DPO's token log-likelihood, arrived at via a bound over noising paths rather than a bare substitution, and what the resulting loss enforces is that the winner x⁺ gains on the frozen reference by MORE than the loser x⁻ does (a relative gap, not each moving in an absolute direction). Trained on 851K Pick-a-Pic pairs, it beat base SDXL on human preference. Subtlety: standard DPO's sigmoid margin is mismatched to the regression nature of image generation (Linear-DPO) — it's not the clean mode-drop you see in LLMs. [rows 12,13,14]
...
| 16 | SaoCriticRoundTrip | static-svg | the baseline's round trip: PPO learns a critic → GRPO deletes it for a group mean → SAO (single rollout, no group) brings the critic back and makes it train reliably (faster updates K=2, frozen attention) | the removed machinery returns for one of the hardest settings |
...
| 19 | PrefGrpoIllusoryAdvantage | static-svg | near-identical pointwise scores ÷ tiny std → blown-up "illusory" advantages (reward hacking); pairwise win rates spread 0-1 → advantages whose size tracks a real ranking | the pairwise verdict returns to settle the frontier |
...
## Codex accuracy review (2026-08-06)

**Fired 2026-08-06** on Vic's "inspect the post and check for accuracy" + "fix and ask codex to check again". Ran AFTER a manual primary-source verification pass (every quote and number re-fetched from arxiv / Nature / the Meta blog / the SAO PDF) that found and fixed four errors of its own: the §12 strongest-GRPO-baseline number, the §10 SAM 3D misquote, the Fig 9 "never build one" overclaim, and the SAM 3 tense-shifted quote.

Codex invocation 1: **9 STRUCTURAL + 7 COSMETIC**. Fifteen applied. Highest-value catches, none of which three prior gates had found:
- The Bradley-Terry through-line was overstated: §2 claimed the sigmoid sits inside "every loss in this post", but GRPO, GSPO and SAO are clipped policy-gradient objectives with no Bradley-Terry term. Narrowed to the preference losses, with an explicit pointer that the policy-gradient branch has no sigmoid.
- §12 called SAO's returned critic "cheap" four times. The paper's own tricks point the other way: K=2 is *more* critic compute, and Frozen-Attention is a stability measure, not a saving; the second policy-sized network's memory bill stands. Reframed to "makes the critic work", with the cost stated.
- pubDate was 2026-06-05 while §12 cites a paper submitted 2026-07-08. Corrected (see the pubDate note above the matrix).
- §3 described PPO as clipping the update to keep the model close. It clips the ratio inside the surrogate objective and removes the incentive to move; nothing pins the weights. Rewritten to the paper's mechanism.
- §7's Diffusion-DPO rungs said the ELBO lower-bounds a probability (it bounds the log-likelihood), that the paper "keeps the DPO loss exactly" (there is a real derivation over noising paths in between), and that each image moves in an absolute direction (the loss acts on the gap). All three corrected.
- §1 taught that SFT fails when nobody can author the single best answer. SFT only needs good-enough demonstrations, and the post's own §10 has SAM 3D using SFT. Reframed to the honest argument: imitation works but caps you at the demonstrator and costs expert time.

**Rejected, 1 finding.** Codex #2 claimed the Shi et al quote should read "depending on **the** type of model mis-specifications". Re-fetched arxiv:2505.19770 twice: the abstract has no article there. The post and matrix row 25 are correct as written; no change made.

**Invocation 2** (via `cxp`, Vic's personal codex): **9 STRUCTURAL + 8 COSMETIC**, all applied. It independently confirmed the Shi rejection was correct. Its theme was that invocation 1's fixes were incomplete: I had fixed prose and captions but left the same claim standing in the SVG text, the aria-label, or the notes copy beside it (Fig 1 "needs one ideal answer", Fig 8 "made more likely", Fig 9 "anyone", Fig 11 "no published per-round numbers", and four "cheap critic" survivals in the notes). Substantive catches beyond that:
- **Pref-GRPO was revised to v2 on 2026-04-20 and all four quotes were stale v1 wording.** See the VERSION DRIFT comment above the matrix. §13 now quotes v2 throughout, which is also more precise.
- The illusory-advantage mental model was wrong in a way the post's own Fig 19 contradicted: dividing by a tiny standard deviation does not produce huge advantages (standardization forces unit variance by construction), it promotes a meaningless gap to a full-strength update. Rewritten, and "honest advantage" softened to what the paper actually shows.
- The DPO implicit reward omitted the prompt-dependent constant β·log Z(x) that cancels in the pairwise difference. Now stated.
- GSPO was credited with making stale-sample reuse "without bias" and with guarantees ("can no longer", "the same range") the paper does not claim. Softened to the paper's language.
- "a billion times a second" for reward-model throughput and the β=0.02 panel label were unsourced flourishes. Both fixed.
- §9 conflated SAM 3's components ("the model now both proposes the label and checks it"); it is a multi-model engine. Corrected.

Full findings from both invocations: [notes/preference-tuning-vision-models-codex-accuracy-20260806.md](preference-tuning-vision-models-codex-accuracy-20260806.md). Totals: inv 1 = 9 STRUCTURAL (8 fixed, 1 rejected with reasoning) + 7 COSMETIC; inv 2 = 9 STRUCTURAL + 8 COSMETIC, all fixed. Two of the three allowed invocations used; a third was not run (Vic called time, and the remaining risk after two passes is low). If a Gate-2-style pass is wanted before ship, that is the slot.
...
| 2026-07-13 | truthfulness re-review (Vic: "review again for accuracy"; primary-source verify + codex) | 4 quote-fidelity findings in §13 Pref-GRPO, all HIGH, all fixed: two paraphrases wrongly wrapped in quotes ("magnified" for the paper's "amplified"; the PPRM "reformulates..." string) + TWO FABRICATED quotes not in the paper at all (the clipping "cannot remove the underlying tendency..." string; "directly suppress[es] the illusory-advantage amplification mechanism" — "suppress" and "magnified" appear nowhere in 2508.20751). Replaced with verbatim abstract quotes ("minimal score differences... amplified after normalization, creating illusory advantages...", "shifts the optimization objective from score maximization to preference fitting", "the win rate is used as the reward signal", "more stable advantages", "mitigating reward hacking"). Re-verified ALL other load-bearing numbers/quotes against primary sources — SAO §12 (K=2, Frozen-Attention, AIME 97.3 vs 84.2, SWE-Bench 29.8 vs 27.0, running-mean 79.8, collapse ~160 / stable ~1000 steps, GLM-5.2 750B-A40B), GSPO, Diffusion-DPO 851K, SAM 3D 5:1 + 400-Elo=10:1, InstructGPT 1.3B-beats-175B + 33k prompts, win-rate formula — all confirmed correct. bun run build + voice-check pass. Codex consult timed out fetching papers (10-min cap); verification completed via direct WebFetch of arxiv abstract + ar5iv body. | inline, this turn |
| 2026-08-06 | accuracy pass (Vic: "check for accuracy" → "fix and ask codex to check again") | structural-fixed. Manual primary-source verification first (4 fixes: §12 wrong GRPO baseline 84.2→93.5 framing, §10 SAM 3D misquote, Fig 9 "never build one", SAM 3 quote tense; plus read-time 28→30 and DAPO attribution). Then codex inv 1: 9 STRUCTURAL + 7 COSMETIC, 15 applied, 1 rejected (Shi "the type" — not in the source, verified twice). Then inv 2 via `cxp` (Vic's personal codex): 9 STRUCTURAL + 8 COSMETIC, all applied, mostly incomplete-fix leftovers in SVG text / aria-labels / notes copies, plus the Pref-GRPO v1→v2 version drift. Inv 2 confirmed the Shi rejection. Tooling note: the `codex` skill the runner documents does not exist; run `CODEX_HOME=~/.codex-personal codex exec --sandbox read-only -c tools.web_search=true -o <file> ... < /dev/null` (backgrounding without closing stdin makes it hang); `-m gpt-5.5` 404s now. 2 of 3 invocations used. | `## Codex accuracy review (2026-08-06)` / notes/preference-tuning-vision-models-codex-accuracy-20260806.md |
