...
3. **The dedicated specialists.** Vision Banana evaluates on five task families per the paper's experiments section: 2D = referring-expression segmentation, semantic segmentation, instance segmentation; 3D = monocular metric depth + surface-normal estimation. Each task family has one or more recent specialist baselines: semantic segmentation lists SAM 3 (2025-11-20) and SegMan-L (the non-zero-shot ceiling); instance segmentation lists DINO-X, OWLv2, Gemini 2.5; referring-expression segmentation lists HybridGL and SAM 3 + Gemini 2.5 Pro; monocular depth lists Depth Pro (2024-10), MoGe-2 (2025-07), UniK3D (2025-03), with Depth Anything 3 (2025-11-13) named separately in paper Table 3; surface normals lists Marigold, StableNormal, DSINE, Lotus-2. Each baseline emits a different output format — binary masks for SAM 3; raw float depth maps for Depth Pro; raw float normal vectors for DSINE — and each was trained on a different per-task pipeline. *The failure mode of this lineup is the interface mismatch:* multiple task families, several output formats, several training stories, all sitting alongside each other. The implicit promise of any "generalist vision model" is to collapse all of those down to one model with one output format and one training pipeline. **Fig 2 (SpecialistLineup):** static-svg. Five task-family columns across the top, each labeled with one or two representative specialist baselines (semantic seg → SAM 3; instance seg → DINO-X; referring-expression seg → HybridGL; depth → UniK3D; normals → Lotus-2). Each column shows that task's output format icon (mask grid / float depth / float normal vectors / etc.). Below them, a downward arrow to a single Vision Banana box that emits one output format (RGB image) for all five families. Reader notices: today's specialists each have a different output-format shape; the question is whether one shape (RGB image) can take over all five families' columns.

### Act 2 — the flip (how Vision Banana works)

4. **Meet Vision Banana.** The architecture overview: Nano Banana Pro (Google's production image generator, released 2025-11-20, = Gemini 3 Pro Image; reached production scale across the Gemini app, Gemini API / Google AI Studio, and Vertex AI) is the substrate. Vision Banana is built by *instruction-tuning* NBP on a mix that adds vision-task data at "a very low ratio" — the paper's exact phrasing. The paper *reports* that the base model's image-generation capabilities are preserved through the tune. The recipe is "minimal task-data uplift on top of an already-deployed generator." This is the paper's *hypothesis*: image-generation pretraining gives a substrate already-suitable for downstream perception, and so a tiny instruction-tune mix is enough to read perception out. The matrix backs the recipe (rows 2, 3, 13); whether the paper's hypothesis is correct is what §8's results test and §14 explicitly leaves open (no random-init ablation). **Fig 3 (VisionBananaArch):** static-svg. NBP at the center as a deep stack with annotation "production generator, deployed via Gemini app / API / Vertex AI"; a small instruction-tune layer wrapped around it; the vision-task data shown as a thin sliver of the training mix; output is an RGB image. Reader notices: the tune is light; the paper *argues* the substrate is pulling most of the weight, but the post must mark this as the paper's argument rather than a settled causal claim.
...
11. **SegGPT (2023): random color mapping per sample.** Same group's follow-up adds a specific mechanism that goes beyond Painter: drop the *fixed* task vocabulary. The training corpus randomly recolors the same segmentation masks across samples — the model never sees a stable "red-means-cat" pairing. It learns "match-this-color-pattern" instead of "label-as-class-N." That's what makes a JSON prompt at inference time work for *any* class set the user supplies, rather than only a fixed canonical set. *Throughline thread:* SegGPT's random-recoloring trick is exactly what allows Vision Banana to swap class palettes per benchmark — the same VB checkpoint can serve different class sets at inference time without retraining the segmentation head. The JSON prompt of §5 is SegGPT's contract made explicit. (No new figure; carries Fig 4 callback in prose.) Reader notices: the prompt isn't decoration; it's the mechanism that makes the same checkpoint work for any class set.
12. **DIFT (2023): correspondence emerges in diffusion features.** Tang et al.: pretrained diffusion models contain *intermediate features* that match the same object across images, with no explicit correspondence supervision. DIFT outperformed DINO and OpenCLIP on SPair-71k by 19 and 14 points respectively. The implication: the generator already learned dense per-pixel representations as a side-effect of denoising — and they transfer to a correspondence task. *Throughline thread:* DIFT is the rung where the *intuition* arrives — that a generator's substrate, if it works for correspondence, plausibly carries enough dense per-pixel structure to underpin downstream dense-prediction tasks like depth and normals. DIFT itself does not validate metric depth or surface-normal transfer; it shows the substrate has the right shape of feature for the dense per-pixel paradigm to be considered at all. Vision Banana then puts that intuition to the test by instruction-tuning depth and normals out of NBP. **Fig 10 (DIFTCorrespondence):** static-svg. Two images of the same animal in different poses; matched correspondence pairs marked with the same color across the pair; an arrow back into the diffusion model showing where these matches come from (intermediate UNet layer features). Reader notices: a model trained only to denoise pictures *also* knows what's the same across pictures — a clue, not yet a proof, that the same features carry dense structure.
13. **Vision Banana (2026): the synthesis.** Final rung. Each predecessor contributes a *mechanism* to the recipe, hedged: MAE established reconstruction-as-pretraining-objective; Painter established that the *output* of every vision task can be an image; SegGPT added the random-color-mapping-per-sample mechanism that lets the same checkpoint serve any class palette; DIFT supplied the intuition that the substrate's intermediate features carry dense per-pixel structure (enough to make dense-prediction transfer plausible at all); Nano Banana Pro (released 2025-11-20, the deployed Gemini 3 Pro Image) supplied production-scale substrate. Add a "very low ratio" of vision-task instruction data on top. Read perception off the generator's RGB output. The paper *argues* that image-generation pretraining serves a role analogous to LLM next-token pretraining; the benchmark spread of §8 is the evidence the paper offers. *Throughline thread:* the cumulative effect maps onto the columns of §8 — semantic seg's per-class color contract is SegGPT plus Painter; the depth and normals columns rest on Painter's "output is image" + DIFT-style features-already-encode-dense-structure intuition + the specific colormaps of §§6–7. The mapping is cumulative across predecessors, not one-predecessor-per-column. **Fig 11 (VisionBananaSynthesis):** static-svg. Five contribution arrows, each labeled with the *specific mechanism the predecessor contributes*: MAE → "reconstruction objective"; Painter → "stitched-image training contract"; SegGPT → "random color mapping per sample"; DIFT → "intermediate features carry dense structure (intuition)"; Nano Banana Pro → "production-scale substrate." Each arrow lands in the relevant Vision Banana mechanism (reconstruction → substrate pretraining; stitched contract → instruction-tune format; random colors → JSON prompts; dense-feature intuition → depth/normals capacity; production substrate → §4 architecture). Reader notices: the timeline isn't decoration; each predecessor contributes a specific mechanism, and the cumulative stack is what makes Vision Banana's recipe minimal.
...
| 2026-05-02 | 1 (outline) — run 3 | **GATE 1 PASSES (cosmetic-only on the gate-runner cap)**. Codex returned 2 findings labeled STRUCTURAL but they were residual editorial inconsistencies from prior runs. Both applied as text-consistency cleanup. | (transcript at `/Users/vic/.claude/projects/-Users-vic-dev-augusteo-com-astro/2afdff51-5714-43ca-9eee-9080739a2dae/tool-results/bzqzq0o15.txt`) |
| 2026-05-02 | 2 (final draft) — run 1 | structural-fixes-applied (9 of 9 STRUCTURAL accepted; 2 COSMETIC accepted). Major findings: §8 wrong instance-seg + RefCOCOg numbers (project-page chart-bar misread, again — the same SA-Co/Gold and RefCOCOg charts as on Cityscapes earlier). §7 + Fig 6 caption normals-encoding self-contradiction (literal [-1,+1]→[0,255] shift would produce R=0 for x=-1, contradicts paper's "Pinkish Red" example). §4 invented training-corpus details. §2/§5 SAM 3 fixed-class-set framing wrong (SAM 3 is concept-prompted; the valid contrast is output contract). §11/§13 SegGPT-causal-for-VB hedge. §3 inline-source-link gaps. §8 + Fig 7 caption inline-source-link gaps. Matrix rows 26-27 freshness annotation date math fixed. §9/§11 throughline tie-backs to §8 chart columns added. Plus "half a year" → "five months." | (transcript at `/Users/vic/.claude/projects/-Users-vic-dev-augusteo-com-astro/2afdff51-5714-43ca-9eee-9080739a2dae/tool-results/bjgvfe612.txt`) |
| 2026-05-02 | 2 (final draft) — run 2 | structural-fixes-applied (1 of 1 STRUCTURAL accepted; 1 COSMETIC accepted). Run 1 missed three leftover [-1,+1]→[0,255] shift / "fixed affine shift per pixel" / "trivial xyz-to-RGB shift" phrasings in §7 + Fig 6 caption + §8 summary line; all replaced with "fixed convention specified in the paper's Methods." Cosmetic: Fig 2 still named DINO-X / HybridGL; replaced with generic "instance" / "refexpr" labels (those names were unsourced and the surrounding prose intentionally avoids them). | (transcript at `/Users/vic/.claude/projects/-Users-vic-dev-augusteo-com-astro/2afdff51-5714-43ca-9eee-9080739a2dae/tool-results/bm2s4g9vk.txt`) |
| 2026-05-02 | 2 (final draft) — run 3 | **GATE 2 PASSES — "no structural issues found."** Codex confirmed the major fixes landed correctly: Cityscapes anchored to VB 0.699 vs SAM 3 0.652; depth averages cleanly separated; surface-normal section no longer asserts the old shift derivation. One cosmetic editorial note: a few figure captions said "the matrix backs..." which read like internal audit scaffolding. Replaced with "the published benchmark numbers refer to..." in Fig 4 + Fig 5 captions. | [notes/generative-vision-stack-codex-final-20260502.md](generative-vision-stack-codex-final-20260502.md) |
