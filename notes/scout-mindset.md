...
- **Act 2 returns** to the GJP repeatedly to ground each mechanism. AOMT is one dispositional finding among several. Brier score is the metric that makes "calibration" measurable. Frequent updating is one behavior associated with accuracy. Each Act-2 section closes by saying "and this is one piece of why the GJP forecasters helped the aggregator outperform".
...
The post will cite Morewedge / Sellier / Swaryandini together: single-session debiasing produces measurable short-term gains (Morewedge); field-transfer is real but modest (Sellier 19%, with corrigendum); and the broader systematic-review picture across 41 studies is small average effects with unresolved real-world transfer (Swaryandini 2025). This is a more honest framing than "debiasing training works".
...
| 18 | In Goldstein et al.'s GJP-vs-ICPM technical paper, the GJP's best-performing aggregation method (the "All Surveys Logit" aggregator) outperformed the U.S. Intelligence Community Prediction Market on a one-year unclassified-question Brier-score benchmark (0-to-2 scale; Codex Run-2 reading: ICPM .23, GJP-best .15). The paper itself notes the "best method" comparison is partly an ex-post selection. **The post does NOT claim "GJP elite teams beat the CIA by 25-30%"** — that public-version framing conflates the algorithmic-aggregation result with the (separately contested) Mellers-2014 individual-intervention claim. | Goldstein, Hartman, Comstock et al. (Good Judgment Inc. technical paper); URL goodjudgment.com/wp-content/uploads/2020/11/Goldstein-et-al-GJP-vs-ICPM.pdf. *(PDF access-restricted in Phase 2 verification; specific Brier numbers cited from Codex Run-2 reading; Phase 7 to verify verbatim.)* | first-party GJP-team paper; current-state aggregation comparison; Phase 7 to verify Brier values |
...
| 24 | Glüer-Pagin & Spectre 2025 conceptual critique: the motivated-numeracy effect involves "plenty of motivation to reason, but no motivated reasoning". The interpretation Kahan offers (identity-protective motivated reasoning as a distortion mechanism) is contested at the level of mechanism, not just empirically. | Glüer-Pagin, K. & Spectre, L. (2025). "Where is the motivation in motivated numeracy?" *Review of Philosophy and Psychology*. DOI 10.1007/s13164-024-00737-w. | current-state / 18-month / passes |
| 25 | Across 41 studies (n = 10,941 students), educational debiasing interventions show a small average effect (*g* = 0.26, 95% CI 0.14-0.39); transfer to real-world decision-making remains unresolved. | Swaryandini, Graham, Griffith, Grilo et al. (2025). "Systematic review and meta-analysis of educational approaches to reduce cognitive biases among students." *Nature Human Behaviour*. DOI 10.1038/s41562-025-02253-y. | current-state / 18-month / passes |
...
Re-running Gate 0 as Run 3 to confirm closure.
...
| 2. Research / fact-check | in progress (Gate 0 Run 2 done with 8 STRUCTURAL findings; corrections applied; Run 3 next; Run 3 is the final allowable Gate 0 invocation per the gate-runner cap) | `## Research notes`, `## Claim-source matrix`, `## Related posts on augusteo.com`, `## Codex research review` |
...
1. Re-run codex Gate 0 (Run 3, final allowable invocation) on the Run-2-corrected matrix. If clean or cosmetic-only, proceed to Phase 3.
2. If Run 3 still surfaces STRUCTURAL findings, the gate-runner Step 6 fires: Vic AskUserQuestion to (a) accept current state and proceed (override; recorded in notes), (b) halt, (c) override on specific findings only.
3. Once Gate 0 closes, build outline + figure list (Phase 3) and run Gate 1.
